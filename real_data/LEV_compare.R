...
get_PLS = function(Data, m = 5, r = 500, task = 'Regression'){
  ...
  # RandomPLS by LEV*
  t1 = Sys.time()
  Prob_lev = LeverageScore(t(X_train), nsketch = 2*ncol(Data$X), eps = 1, verbose = FALSE)
  B <- RandomPLS(X_train, Y_train, m, Prob_lev, r)
  t2 = Sys.time()
  Y_pred = X_test %*% B
  if(task == 'Regression'){
  ...
  # SparPLS by LEV
  t1 = Sys.time()
  Prob_spar = abs(X_train) * RowProb(X_train, Y_train, option = 'lev')[,1]
  B = SparPLS(X_train, Y_train, m, Prob_spar,r)
  t2 = Sys.time()
  Y_pred = X_test %*% B
  if(task == 'Regression'){
  ...
  # SparPLS by LEV*
  t1 = Sys.time()
  Prob_spar = abs(X_train) * LeverageScore(t(X_train), nsketch = 2*ncol(Data$X), eps = 1, verbose = FALSE)
  Prob_spar = Prob_spar / sum(Prob_spar)
  B = SparPLS(X_train, Y_train, m, Prob_spar,r)
  t2 = Sys.time()
  Y_pred = X_test %*% B
  if(task == 'Regression'){
