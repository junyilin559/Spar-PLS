...
get_PLS = function(Data, m = 5, r = 500, task = 'Regression'){
  ...
  # SGDPLS
  results = SGDPLS(X_train, Y_train, m)
  B = results$Beta
  Y_pred = X_test %*% B
  if(task == 'Regression'){
  ...
  # CIPLS
  results = CIPLS(X_train, Y_train, m)
  B = results$Beta
  Y_pred = X_test %*% B
  if(task == 'Regression'){
    mse[7] = mean((Y_pred - Y_test)^2) 
  }else{
    mse[7] = mean(apply(Y_pred, 1, which.max) == apply(Y_test, 1, which.max)) 
  }
  t[7] = results$time
  ...
  Y_pred = X_test %*% B
  if(task == 'Regression'){
    mse[8] = mean((Y_pred - Y_test)^2) 
  }else{
    mse[8] = mean(apply(Y_pred, 1, which.max) == apply(Y_test, 1, which.max)) 
  }
  t[8] = as.numeric(difftime(t2, t1, units = "secs"))
