# source: https://github.com/arturjordao/IncrementalDimensionalityReduction/blob/master/Code/CIPLS.py
...
class CIPLS(BaseEstimator):
    ...
    def __init__(self, n_components=5, copy=True):
    ...
    def get_transform(self, X, Y=None, copy=True):
...
def main():
    parser = argparse.ArgumentParser(description='SGDPLS')
    parser.add_argument('--m', type=int, default=5, help='Number of components')
    args = parser.parse_args()
    m = args.m
    if(m > 5):
        print("CIPLS Warning: High number of components may lead to numerical instability.")
        m = 5

    X_train = pd.read_csv("WEC_X_train.csv").values
    Y_train = pd.read_csv("WEC_Y_train.csv").values

    t1 = time.time()
    Latent_Param = np.zeros([X_train.shape[1], m * Y_train.shape[1]])
    for i in range(Y_train.shape[1]): 
        model_cipls = CIPLS(n_components=m)
        model_cipls.fit(X_train, Y_train[:, i]) 
        idx = i*m 
        Latent_Param[:, idx:idx+m] = model_cipls.x_rotations.T
    model = LinearRegression(fit_intercept=False)
    model.fit(X_train @ Latent_Param, Y_train)
    Beta = Latent_Param @ model.coef_.T
    t2 = time.time()
    delta_t = t2 - t1
    np.savetxt("CIPLS_Beta.csv", Beta, delimiter=",")
    np.savetxt("CIPLS_time.txt", np.array([delta_t]))
