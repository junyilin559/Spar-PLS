# source: https://github.com/arturjordao/IncrementalDimensionalityReduction/blob/master/Code/SGDPLS.py
...
def main():
    ...
    parser.add_argument('--location', type=str, default='Perth', help='Location of the WEC Experiment')
    parser.add_argument('--n_wec', type=int, default=49, help='Number of WECs')
    parser.add_argument('--m', type=int, default=5, help='Number of components')
    parser.add_argument('--epochs', type=int, default=10, help='Number of epochs')
    parser.add_argument('--eta', type=float, default=0.0001, help='Learning rate')
    ...
    # df = pd.read_csv(f"data/WEC_{location}_{n_wec}.csv")
    X_train = pd.read_csv("WEC_X_train.csv")
    Y_train = pd.read_csv("WEC_Y_train.csv")

    t1 = time.time()
    model = SGDPLS(n_components=m, epochs=epochs, eta=eta)
    model.fit(X_train, Y_train)
    Beta = model.x_rotations @ np.linalg.pinv(model.y_rotations)
    t2 = time.time()
    delta_t = t2 - t1
    np.savetxt("SGDPLS_Beta.csv", Beta, delimiter=",")
    np.savetxt("SGDPLS_time.txt", np.array([delta_t]))
